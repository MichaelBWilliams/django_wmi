...
var adm1Mini = L.geoJSON(EC_adm1, {
	style: adm1StyleMini
});
var layers = new L.LayerGroup([osm2, adm1Mini]);
var miniMap = new L.Control.MiniMap(layers, {toggleDisplay: true, zoomLevelFixed: 5, centerFixed: [-1.421, -78.588]}).addTo(map);
// End mini map plugin
...
// Layers
var adm1Layer = L.geoJSON(EC_adm1, {
	style: adm1Style, 
	onEachFeature: onEachFeature
}).bindTooltip(function(layer){return String('<b>' + layer.feature.properties.name + '</b>')}, {direction: "center", className: "admLabel"});


var exposureLayer = L.geoJSON(EC_adm1, {
	style: styleExposure, 
	onEachFeature: onEachFeature2
}).bindTooltip(function(layer){return String('<b>' + layer.feature.properties.name + ': ' + (layer.feature.properties.percent) + '% Exposure' + '</b>')}, {direction: "center", className: "admLabel"});


var branchLayer = L.geoJSON(HN_branchList, {
}).bindPopup(function(layer){return String('<b>' + layer.feature.properties.name + '</b><br/>' + 'Outstanding Portfolio 2016 (USD): <br/>' + 'Total: <b>' + (format2(layer.feature.properties.Outstandin, "$")) + '</b><br/>' + 'Agriculture: <b>' + (format2(layer.feature.properties.Agricultur, "$")) + '</b><br/>' + 'Commerce: <b>' + (format2(layer.feature.properties.Commerce, "$")) + '</b><br/>' + 'Industry: <b>' + (format2(layer.feature.properties.Industry, "$")) + '</b><br/>'  + 'Services: <b>' + (format2(layer.feature.properties.Services, "$")) + '</b>')}, {direction: "center", className: "admLabel"});
...
infoExposure.update = function (props) {
	this._div.innerHTML = '<h4>Selected Administrative Unit</h4>' + (props ? 'Name: ' + '<b>' + props.name + '</b><br/>' + 'GID # ' + '<b>' + props.gid + '</b><br/>' + 'Estimated % of<br/>Total Exposure: ' + '<b>' + props.percent + '%</b><br/>' + '2016 Exposure<br/>in USD: ' + '<b>' + (format2(props.exposure_u, "$")) + '</b>' : 'Hover to see:<br/><b>Administrative unit name<br/>GID #<br/>Estimated exposure values</b>');
};
map.on('overlayadd', function (eventLayer) {
	if (eventLayer.name === "Admin 1 Exposure"){
		console.log("boop2");
